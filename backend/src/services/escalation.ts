...
/**
 * Sinyal lingkungan per kecamatan — bahan lapisan pemicu di peta (PRD §4, S1).
 *
 * Yang dihitung hanya laporan **terverifikasi** berkeluarga lingkungan:
 * genangan, sampah, saluran. Berbeda dari eskalasi di atas yang sengaja ikut
 * menghitung laporan yang masih menunggu — sebuah lapisan peta yang dibaca
 * sebagai fakta lapangan tidak boleh berisi laporan yang belum diperiksa
 * siapa pun.
 *
 * Dan yang tidak boleh disalahbaca: ini **peta laporan warga**, bukan peta
 * genangan. Kecamatan tanpa penanda berarti tidak ada laporan terverifikasi
 * di sana, bukan berarti kering.
 */
...
export async function environmentSignals(
  windowDays = 60,
): Promise<{ windowDays: number; signals: EnvironmentSignal[] }> {
  ...
  const rows = await all<{
    ...
    `SELECT kecamatan, kind, occurred_at
       FROM laporan_warga
  ...
  const byKecamatan = new Map<string, EnvironmentSignal>();
  for (const row of rows) {
    const entry =
      byKecamatan.get(row.kecamatan) ??
      ({
    ...
    entry.total += 1;
    if (row.kind === "genangan") entry.genangan += 1;
    if (row.kind === "sampah") entry.sampah += 1;
    if (row.kind === "saluran") entry.saluran += 1;
    if (!entry.terakhir || row.occurred_at > entry.terakhir) {
      entry.terakhir = row.occurred_at;
    }
  ...
  return {
    windowDays,
    signals: [...byKecamatan.values()].sort((a, b) => b.total - a.total),
  };
}
