...
export function AppleGlassDate({
  ...
  return (
    ...
    >
      ...
      <div className="relative inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-white/90 text-brand-900 border border-white/90 shadow-[0_1px_2px_rgba(14,34,37,0.05),inset_0_1px_0_rgba(255,255,255,1)] font-semibold text-caption tracking-tight shrink-0">
        <span>{primary}</span>
      </div>
      ...
      {dateRange && (
        <>
          <span className="text-paper-300 font-light select-none text-xs">·</span>
          <span className="relative text-caption font-medium text-paper-600 font-mono whitespace-nowrap">
            {dateRange}
          </span>
        </>
      )}
      ...
      {showLivePulse && (
        <div className="relative hidden sm:inline-flex items-center gap-1.5 pl-2 pr-2.5 py-0.5 rounded-full bg-risk-low-bg border border-risk-low-br text-risk-low text-caption font-medium tracking-tight shrink-0 shadow-xs">
          ...
          <span className="font-medium text-caption">{liveLabel}</span>
        </div>
      )}
    </div>
  );
