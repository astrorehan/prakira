...
export function PeriodChip({ className }: { className?: string }) {
  return (
    <span
      role="status"
      aria-label={`Periode data: ${REPORTING_PERIOD.weekLabel}, ${REPORTING_PERIOD.monthYear}`}
      className={cn(
    ...
    >
      ...
      <span className="tabular text-caption font-medium text-foreground">
        {REPORTING_PERIOD.weekLabel}
      </span>
      <span aria-hidden="true" className="text-paper-300">
        ·
      </span>
      <span className="text-caption text-paper-600">{REPORTING_PERIOD.monthYear}</span>
    </span>
  );
}
